...
function ba_sw_set_suggested_word() {
    ...
    if (ba_sw_read_cookie(cookie_name) === null) {
        ...
        $(".define-form .look_up a").attr("href", tracer_url_for_unique);
    }
    else {
        $(".define-form .look_up a").attr("href", tracer_url_for_word);
    }

    $(".define-form .look_up a").html(word);
}
...
function ba_sw_init_submit_btn() {
    $(".define-form .input-append input.btn").unbind();
    $(".define-form .input-append input.btn").click(function() {
        var word = $("#term").val();
...
function ba_sw_stat(id, url) {
    ...
    if (url !== '') {
        ...
        $("#" + id).html('<img src="' + tracer_url + '" style="width: 1px; height: 1px; border: none;" />');
    }
}
